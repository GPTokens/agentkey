...
fn acquire_single_instance_guard(
    debug_port: u16,
) -> anyhow::Result<Option<agentkey_core::ports::LoopbackPortGuard>> {
...
fn log_launcher_guard_fallback(fallback_lock_path: &Path) {
    let _ = agentkey_core::diagnostic_log::append_diagnostic_log(
        ...
        json!({
            "requested_guard_port": agentkey_core::ports::LAUNCHER_GUARD_PORT,
...
fn should_recover_stale_launcher(debug_port: u16) -> bool {
    let has_codex_process = !agentkey_core::watcher::find_codex_processes().is_empty();
    let cdp_listening = agentkey_core::watcher::cdp_listening(debug_port);
    let recover =
        agentkey_core::watcher::should_recover_stale_launcher(has_codex_process, cdp_listening);
    let _ = agentkey_core::diagnostic_log::append_diagnostic_log(
...
async fn activate_existing_codex_app(options: &LaunchOptions) -> anyhow::Result<()> {
    ...
    if settings.enhancements_enabled {
        hooks
            .start_helper(options.helper_port, helper_token.clone())
            .await?;
    }
    let process_ids = agentkey_core::watcher::find_codex_processes();
    let mut activated = false;
    #[cfg(windows)]
    {
        for process_id in &process_ids {
            if agentkey_core::windows_activate_process_window(*process_id) {
                activated = true;
                break;
            }
        }
    ...
    if injection_ready {
        hooks
            .start_bridge_watchdog(options.debug_port, options.helper_port, &helper_token)
            .await?;
...
fn log_launcher_already_running(debug_port: u16) {
    let _ = agentkey_core::diagnostic_log::append_diagnostic_log(
        ...
        json!({
            "guard_port": agentkey_core::ports::LAUNCHER_GUARD_PORT,
            "debug_port": debug_port
        }),
    );
}
...
impl LaunchHooks for LauncherHooks {
    fn resolve_app_dir(
        ...
        settings: &agentkey_core::settings::BackendSettings,
    ...
    async fn load_settings(&self) -> anyhow::Result<agentkey_core::settings::BackendSettings> {
        self.core.load_settings().await
    }

    async fn run_provider_sync(&self) -> anyhow::Result<()> {
        let _ = tokio::task::spawn_blocking(|| agentkey_data::run_provider_sync(None))
            .await
    ...
    async fn start_helper(&self, helper_port: u16, helper_token: String) -> anyhow::Result<()> {
        self.core.start_helper(helper_port, helper_token).await
    }
    ...
    async fn launch_codex(
        ...
        debug_port: u16,
        extra_args: &[String],
    ) -> anyhow::Result<agentkey_core::launcher::CodexLaunch> {
        self.core
    ...
    async fn wait_for_codex_exit(
        &self,
        launch: &agentkey_core::launcher::CodexLaunch,
    ) -> anyhow::Result<()> {
    ...
    async fn terminate_codex(&self, launch: &agentkey_core::launcher::CodexLaunch) {
...
impl Default for LauncherDataService {
    fn default() -> Self {
        Self {
            ...
            backup_dir: agentkey_core::paths::default_app_state_dir().join("backups"),
        }
    }
}
...
impl LauncherDataService {
    fn storage_adapter(&self) -> agentkey_data::SQLiteStorageAdapter {
        agentkey_data::SQLiteStorageAdapter::new(
            self.db_path.clone(),
            agentkey_data::BackupStore::new(self.backup_dir.clone()),
        )
    }
}
...
impl BridgeRuntimeService for LauncherRuntimeService {
    ...
    async fn reload_user_scripts(&self) -> anyhow::Result<Value> {
        ...
        if let Some(websocket_url) = websocket_url.filter(|_| !bundle.trim().is_empty()) {
            agentkey_core::bridge::evaluate_script(&websocket_url, &bundle).await?;
        }
        self.user_scripts.inventory()
    }
    ...
    async fn open_devtools(&self) -> anyhow::Result<Value> {
        ...
        let targets = agentkey_core::cdp::list_targets(debug_port).await?;
        let target = agentkey_core::cdp::pick_page_target(&targets)?;
        let url = agentkey_core::routes::devtools_url(debug_port, &target.id);
        open_url(&url)?;
        Ok(json!({
            "status": "ok",
            "target_id": target.id,
    ...
    async fn open_manager(&self) -> anyhow::Result<Value> {
        ...
        #[cfg(windows)]
        {
            std::process::Command::new(&manager_path)
                .creation_flags(agentkey_core::windows_create_no_window())
                .spawn()
                .map_err(|error| anyhow::anyhow!("启动管理工具失败：{error}"))?;
        }
        #[cfg(not(windows))]
    ...
    async fn backend_status(&self) -> anyhow::Result<Value> {
        Ok(
            json!({"status": "ok", "message": "后端已连接", "version": agentkey_core::version::VERSION}),
        )
    }
    ...
    async fn codex_model_catalog(&self) -> anyhow::Result<Value> {
        Ok(agentkey_core::model_catalog::read_codex_model_catalog().await)
    }

    async fn ads(&self) -> anyhow::Result<Value> {
        agentkey_core::ads::fetch_ad_list().await
    }

    async fn zed_remote_status(&self) -> anyhow::Result<Value> {
        Ok(agentkey_core::zed_remote::zed_remote_status())
    }

    async fn resolve_zed_remote_host(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(agentkey_core::zed_remote::resolve_ssh_target_response(
            &payload,
        ))
    }

    async fn fallback_zed_remote_request(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(agentkey_core::zed_remote::fallback_open_request_response(
            &payload,
        ))
    }

    async fn open_zed_remote(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(agentkey_core::zed_remote::open_zed_remote(&payload))
    }

    async fn upstream_worktree_status(&self) -> anyhow::Result<Value> {
        Ok(agentkey_core::upstream_worktree::status_response())
    }

    async fn upstream_worktree_defaults(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(agentkey_core::upstream_worktree::defaults_response(
            &payload,
        ))
    }
...
async fn try_inject_with_context(
    ...
    let script = agentkey_core::assets::injection_script(helper_port, helper_token);
    ...
    agentkey_core::bridge::install_bridge(
        websocket_url,
        agentkey_core::bridge::BRIDGE_BINDING_NAME,
        Arc::new(move |path, payload| {
            let ctx = ctx.clone();
            Box::pin(async move {
                Ok(agentkey_core::routes::handle_bridge_request(ctx, &path, payload).await)
            })
        }),
...
fn manager_exe_path() -> PathBuf {
    ...
    dir.join(format!(
        "{}{}",
        agentkey_core::install::MANAGER_BINARY,
        suffix
    ))
}
...
fn default_user_scripts_config_dir() -> PathBuf {
    if cfg!(windows) {
        ...
        if let Some(home) = directories::BaseDirs::new().map(|dirs| dirs.home_dir().to_path_buf()) {
            return home.join("AppData").join("Roaming").join("AgentKey");
        }
    }
    ...
    fn manager_update_prompt_uses_sidecar_manager_binary_name() {
        ...
        assert!(
            path.file_name()
                ...
                .is_some_and(|name| name.contains(agentkey_core::install::MANAGER_BINARY))
        );
    }
}
