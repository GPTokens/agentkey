...
fn acquire_single_instance_guard() -> Option<agentkey_core::ports::LoopbackPortGuard> {
    match agentkey_core::ports::acquire_resilient_loopback_port_guard(
        agentkey_core::ports::MANAGER_GUARD_PORT,
    ) {
        Ok(guard) => {
            if let Some(fallback_lock_path) = guard.fallback_path() {
                let _ = agentkey_core::diagnostic_log::append_diagnostic_log(
                    ...
                    serde_json::json!({
                        "requested_guard_port": agentkey_core::ports::MANAGER_GUARD_PORT,
        ...
        Err(error) if error.kind() == std::io::ErrorKind::AddrInUse => {
            let _ = agentkey_core::diagnostic_log::append_diagnostic_log(
                ...
                serde_json::json!({
                    "guard_port": agentkey_core::ports::MANAGER_GUARD_PORT
                }),
            );
            None
        }
