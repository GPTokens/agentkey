...
use agentkey_core::install::SILENT_BINARY;
use agentkey_core::models::{DeleteResult, SessionRef};
use agentkey_core::script_market::{self, MarketScript, ScriptMarketManifest};
use agentkey_core::settings::{BackendSettings, RelayProfile, SettingsStore};
use agentkey_core::status::{LaunchStatus, StatusStore};
...
pub struct LocalSessionsPayload {
    pub db_path: String,
    pub sessions: Vec<agentkey_data::LocalSession>,
}
...
pub struct ContextEntriesPayload {
    pub settings: BackendSettings,
    pub entries: agentkey_core::relay_config::CodexContextEntries,
}
...
pub fn backend_version() -> CommandResult<VersionPayload> {
    ok(
        "后端版本已读取。",
        VersionPayload {
            version: agentkey_core::version::VERSION.to_string(),
        },
    )
}
    ...
    let Ok((codex_app_path, entrypoints, latest_launch)) = payload else {
        return failed(
            ...
            OverviewPayload {
                ...
                latest_launch: None,
                current_version: agentkey_core::version::VERSION.to_string(),
                update_status: "not_checked".to_string(),
                settings_path: agentkey_core::paths::default_settings_path()
                    .to_string_lossy()
                    .to_string(),
                logs_path: agentkey_core::paths::default_diagnostic_log_path()
                    .to_string_lossy()
                    .to_string(),
            },
        );
...
#[tauri::command]
pub fn launch_agentkey(request: LaunchRequest) -> CommandResult<Value> {
    spawn_agentkey_launch(request, "启动任务已在后台开始，可稍后查看概览状态。")
}
...
fn spawn_silent_launcher(request: &LaunchRequest) -> anyhow::Result<()> {
    let launcher = agentkey_core::install::companion_binary_path(SILENT_BINARY);
...
pub fn list_local_sessions() -> CommandResult<LocalSessionsPayload> {
    let db_path = agentkey_core::relay_config::default_codex_home_dir().join("state_5.sqlite");
...
pub fn delete_local_session(request: DeleteLocalSessionRequest) -> CommandResult<DeleteResult> {
    ...
    if session_id.is_empty() {
        return failed(
            "会话 ID 不能为空。",
            DeleteResult {
                status: agentkey_core::models::DeleteStatus::Failed,
    ...
    let status = if matches!(
        result.status,
        agentkey_core::models::DeleteStatus::LocalDeleted
    ) {
        "ok"
    } else {
        "failed"
...
fn local_session_adapter(db_path: &Path) -> agentkey_data::SQLiteStorageAdapter {
    agentkey_data::SQLiteStorageAdapter::new(
        db_path,
        agentkey_data::BackupStore::new(
            agentkey_core::paths::default_app_state_dir().join("backups"),
        ),
    )
}
...
fn normalize_settings_before_save(mut settings: BackendSettings) -> BackendSettings {
    if let Some(path) =
        agentkey_core::app_paths::normalize_codex_app_path(Path::new(&settings.codex_app_path))
    {
        settings.codex_app_path = path.to_string_lossy().to_string();
    }
    settings.relay_common_config_contents =
        agentkey_core::relay_config::sanitize_common_config_contents(
            &settings.relay_common_config_contents,
        );
    ...
    for profile in &mut settings.relay_profiles {
        if let Err(error) =
            agentkey_core::relay_config::normalize_relay_profile_for_storage(profile)
        {
            log_manager_event(
    ...
    if !common_config.trim().is_empty() {
        for profile in &mut settings.relay_profiles {
            ...
            match agentkey_core::relay_config::strip_common_config_from_config(
...
fn remove_linked_ccs_profiles_for_local_storage(settings: &mut BackendSettings) {
            ...
            .any(|profile| profile.id == settings.active_relay_id)
    {
        settings.active_relay_id = settings
            ...
            .unwrap_or_else(agentkey_core::settings::default_active_relay_id);
    }
}
...
fn merge_manual_provider_sync_targets(
    targets: &mut agentkey_data::ProviderSyncTargetList,
    manual: &[String],
    ...
    for id in manual {
        if let Some(existing) = targets.targets.iter_mut().find(|target| target.id == *id) {
            if !existing
                .sources
                .contains(&agentkey_data::ProviderSyncTargetSource::Manual)
            {
                existing
                    .sources
                    .push(agentkey_data::ProviderSyncTargetSource::Manual);
            ...
            targets
                .targets
                .push(agentkey_data::ProviderSyncTargetOption {
                    id: id.clone(),
                    sources: vec![agentkey_data::ProviderSyncTargetSource::Manual],
...
pub async fn sync_providers_now(target_provider: Option<String>) -> CommandResult<Value> {
    ...
    let result = tauri::async_runtime::spawn_blocking(move || {
        agentkey_data::run_provider_sync_with_target(None, target_provider.as_deref())
    })
    .await
...
fn is_success_sync_status(status: &agentkey_data::ProviderSyncStatus) -> bool {
    matches!(status, agentkey_data::ProviderSyncStatus::Synced)
}
...
pub async fn load_ads() -> CommandResult<AdsPayload> {
    match agentkey_core::ads::fetch_ad_list().await {
...
pub async fn check_update() -> CommandResult<Value> {
    match agentkey_core::update::check_for_update(agentkey_core::version::VERSION).await {
        Ok(update) => {
        ...
        Err(error) => failed(
            &format!("检查更新失败：{error}"),
            json!({
                "currentVersion": agentkey_core::version::VERSION,
...
pub async fn perform_update(
    release: Option<agentkey_core::update::Release>,
) -> CommandResult<Value> {
    ...
    let download_dir = agentkey_core::paths::default_app_state_dir().join("updates");
    match agentkey_core::update::perform_update(&release, &download_dir).await {
        Ok(result) => ok(
...
pub fn install_watcher() -> CommandResult<WatcherPayload> {
    let launcher_path =
        agentkey_core::install::companion_binary_path(agentkey_core::install::SILENT_BINARY);
    match agentkey_core::watcher::install_watcher(&launcher_path, default_debug_port()) {
...
pub fn uninstall_watcher() -> CommandResult<WatcherPayload> {
    match agentkey_core::watcher::uninstall_watcher() {
...
pub fn enable_watcher() -> CommandResult<WatcherPayload> {
    match agentkey_core::watcher::enable_watcher() {
...
pub fn disable_watcher() -> CommandResult<WatcherPayload> {
    match agentkey_core::watcher::disable_watcher() {
...
pub fn read_latest_logs(request: LogRequest) -> CommandResult<LogsPayload> {
    let path = agentkey_core::paths::default_diagnostic_log_path();
...
pub fn write_diagnostic_event(event: String, detail: Value) -> CommandResult<Value> {
    ...
    match agentkey_core::diagnostic_log::append_diagnostic_log(&event, detail) {
        Ok(()) => ok("诊断日志已写入。", json!({})),
...
pub fn backfill_relay_profile_from_live(
    ...
    match agentkey_core::relay_config::backfill_relay_profile_from_home_with_common(
        &home,
        profile,
...
pub fn list_context_entries(
    ...
    match agentkey_core::relay_config::list_context_entries_from_common_config(
...
pub fn read_live_context_entries() -> CommandResult<LiveContextEntriesPayload> {
    ...
    match agentkey_core::relay_config::list_context_entries_from_common_config(&config) {
        Ok(entries) => ok(
            "live 工具与插件已读取。",
...
pub fn upsert_context_entry(request: ContextEntryRequest) -> CommandResult<ContextEntriesPayload> {
    let mut settings = request.settings;
    match agentkey_core::relay_config::upsert_context_entry_in_common_config(
        &settings.relay_context_config_contents,
        &request.kind,
        &request.id,
        &request.toml_body,
...
pub fn sync_live_context_entries(
    ...
    let updated_config = match agentkey_core::relay_config::sync_live_config_context_entries(
        &current_config,
        &request.settings.relay_context_config_contents,
    ) {
        Ok(config) => config,
    ...
    match agentkey_core::relay_config::list_context_entries_from_common_config(&updated_config) {
        Ok(entries) => ok(
            "live 工具与插件已同步。",
            LiveContextEntriesPayload { entries },
        ),
...
pub fn delete_context_entry(request: ContextDeleteRequest) -> CommandResult<ContextEntriesPayload> {
    let mut settings = request.settings;
    match agentkey_core::relay_config::delete_context_entry_from_common_config(
        &settings.relay_context_config_contents,
        &request.kind,
        &request.id,
    ) {
...
pub fn extract_relay_common_config(
    ...
    match agentkey_core::relay_config::extract_common_config_from_config(&request.config_contents)
        .and_then(|common_config_contents| {
            let profile_config_contents =
                agentkey_core::relay_config::strip_common_config_from_config(
...
pub async fn test_relay_profile(profile: RelayProfile) -> CommandResult<RelayProfileTestPayload> {
    ...
    match agentkey_core::relay_config::test_relay_profile(&profile, test_model).await {
        Ok(result) => {
...
pub async fn fetch_relay_profile_models(
    ...
    match agentkey_core::model_catalog::fetch_relay_profile_model_ids(&profile).await {
...
pub fn apply_relay_injection() -> CommandResult<RelayPayload> {
    ...
    let auth = agentkey_core::relay_config::chatgpt_auth_status_from_home(&home);
    ...
    match agentkey_core::relay_config::apply_relay_config_to_home_with_protocol(
        &home,
        &relay.base_url,
        &relay.api_key,
        relay.protocol,
        agentkey_core::protocol_proxy::DEFAULT_PROTOCOL_PROXY_PORT,
    ) {
        Ok(result) => {
            let status = agentkey_core::relay_config::relay_status_from_home(&home);
            log_relay_apply_result(
...
pub fn apply_pure_api_injection() -> CommandResult<RelayPayload> {
    ...
    match agentkey_core::relay_config::apply_pure_api_config_to_home_with_protocol(
        &home,
        &relay.base_url,
        &relay.api_key,
        relay.protocol,
...
fn relay_has_complete_files(relay: &agentkey_core::settings::RelayProfile) -> bool {
    if relay.relay_mode == agentkey_core::settings::RelayMode::Official
        && relay.official_mix_api_key
    {
...
fn log_relay_apply_request(
    event: &str,
    settings: &BackendSettings,
    relay: &agentkey_core::settings::RelayProfile,
) {
    let _ = agentkey_core::diagnostic_log::append_diagnostic_log(
        event,
        json!({
...
fn log_relay_apply_result(
    event: &str,
    relay: &agentkey_core::settings::RelayProfile,
    status: &agentkey_core::relay_config::RelayStatus,
...
fn log_manager_event(event: &str, detail: Value) {
    let _ = agentkey_core::diagnostic_log::append_diagnostic_log(event, detail);
}
...
fn empty_context_entries() -> agentkey_core::relay_config::CodexContextEntries {
    agentkey_core::relay_config::CodexContextEntries {
        mcp_servers: Vec::new(),
        skills: Vec::new(),
        plugins: Vec::new(),
    }
...
fn open_url(url: &str) -> anyhow::Result<()> {
    #[cfg(windows)]
    {
        agentkey_core::windows_open_url(url)
    }
    #[cfg(not(windows))]
    {
...
fn settings_payload_value() -> Result<SettingsPayload, (anyhow::Error, SettingsPayload)> {
    let store = SettingsStore::default();
    let settings_path = agentkey_core::paths::default_settings_path()
        .to_string_lossy()
        .to_string();
    match store.load() {
...
fn user_scripts_config_dir() -> PathBuf {
    if cfg!(windows) {
        if let Some(roaming) = std::env::var_os("APPDATA") {
            return PathBuf::from(roaming).join("AgentKey");
        }
    }
...
fn diagnostics_report() -> String {
    ...
    let report = agentkey_core::diagnostic_log::redact_diagnostic_value(json!({
        ...
        "settings": settings,
        "logs": {
            "diagnosticLogPath": agentkey_core::paths::default_diagnostic_log_path(),
            "latestStatusPath": agentkey_core::paths::default_latest_status_path()
        },
        "platform": {
...
fn load_overview_payload() -> (
    ...
    let settings = SettingsStore::default().load().unwrap_or_default();
    (
        agentkey_core::app_paths::resolve_codex_app_dir_with_saved(
            None,
...
fn watcher_payload() -> WatcherPayload {
    let flag = agentkey_core::watcher::default_watcher_disabled_flag();
    WatcherPayload {
        enabled: !flag.exists(),
...
mod tests {
    ...
    fn startup_options_honors_show_update_argument() {
        assert!(should_show_update(
            ["agentkey-manager.exe", "--show-update"],
            None
        ));
    }
    ...
    fn relay_payload_does_not_expose_token_text() {
        let payload = relay_payload(
            agentkey_core::relay_config::RelayStatus {
                authenticated: true,
    ...
    fn apply_relay_profile_to_home_with_switch_rules_preserves_custom_provider_id() {
        ...
        let profile = RelayProfile {
            relay_mode: agentkey_core::settings::RelayMode::PureApi,
            protocol: agentkey_core::settings::RelayProtocol::Responses,
        ...
        agentkey_core::relay_config::apply_relay_profile_to_home_with_switch_rules(
            temp.path(),
            &profile,
            "",
        )
    ...
    fn normalize_settings_before_save_preserves_official_profile_auth() {
        let settings = BackendSettings {
            relay_profiles: vec![RelayProfile {
                relay_mode: agentkey_core::settings::RelayMode::Official,
