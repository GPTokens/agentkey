...
export function App() {
  ...
  const launch = async () => {
    const result = await launchCommand("launch_agentkey");
    if (result) {
  ...
  const restart = async () => {
    const result = await launchCommand("restart_agentkey");
    if (result) {
      showNotice("重启 AgentKey", result.message, result.status);
      await refreshOverview(true);
    }
  };

  const launchCommand = async (command: "launch_agentkey" | "restart_agentkey") => {
    const result = await run(() =>
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem("agentkey-theme", theme);
  }, [theme]);
        ...
        <div className="brand">
          ...
          <div className="brand-copy">
            <div className="brand-title-row">
              <div className="brand-title">AgentKey</div>
              {hasUpdate ? (
                <button
      ...
      <main className="workspace">
        <header className="topbar" key={`topbar-${route}`}>
          ...
          <div className="topbar-actions">
            ...
            <Button onClick={() => void actions.restart()} title="重启 AgentKey" variant="outline">
      ...
      <Panel>
        ...
        <CardContent>
          <label className="switch-row">
            ...
            />
            <span>
              <strong>启用 AgentKey 页面增强</strong>
        ...
        <CardContent>
          ...
          <label className="switch-row">
            ...
            <span>
              ...
              <small>开启后，通过 AgentKey 启动 Codex 前自动整理一次旧对话的归属标记。</small>
            </span>
          </label>
          <Toolbar>
        ...
        <CardContent>
          <div className="recommend-hero">
            <div>
              ...
              <span>内容来自 GPTokens/agentkey，分为赞助商推荐和普通推荐。</span>
            </div>
...
function MaintenanceScreen({
  ...
  return (
    <>
      ...
      <Panel>
        <CardHead title="自动接管" detail="Watcher 用于保持 AgentKey 接管状态" />
        <CardContent>
          <Toolbar>
      ...
      <Panel>
        ...
        <CardContent>
          ...
          <Toolbar>
            <Button onClick={() => void actions.launch()}>启动 AgentKey</Button>
...
function healthItems(overview: OverviewResult | null) {
  return [
    ...
    },
    {
      ...
      detail: overview?.silent_shortcut.path || "缺少 AgentKey 静默启动快捷方式时可在安装维护页修复。",
    },
    {
      title: "管理工具入口",
...
function loadInitialTheme(): Theme {
  ...
  return window.localStorage.getItem("agentkey-theme") === "light" ? "light" : "dark";
}
