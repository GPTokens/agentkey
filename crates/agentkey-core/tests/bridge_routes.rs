...
use agentkey_core::models::{DeleteResult, DeleteStatus, ExportResult, ExportStatus, SessionRef};
use agentkey_core::routes::{
...
async fn runtime_status_devtools_repair_and_ads_routes_are_dispatched() {
    ...
    assert_eq!(
        ...
        json!({"status": "ok", "message": "后端已修复", "version": agentkey_core::version::VERSION})
    );
    assert_eq!(
...
fn script_market_manifest_filters_invalid_entries() {
    ...
    let manifest = agentkey_core::script_market::parse_market_manifest(raw).unwrap();
...
fn user_script_inventory_includes_market_metadata() {
    ...
    manager
        .record_market_install(&agentkey_core::script_market::MarketScript {
            id: "demo".to_string(),
            name: "Demo".to_string(),
            description: "Useful demo".to_string(),
            version: "1.0.0".to_string(),
            author: "GPTokens".to_string(),
...
impl LaunchHooks for ContextHooks {
    ...
    async fn start_helper(&self, _helper_port: u16, _helper_token: String) -> anyhow::Result<()> {
        Ok(())
    }
