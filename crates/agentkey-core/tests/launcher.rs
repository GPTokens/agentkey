...
#[tokio::test]
async fn default_helper_rejects_backend_status_without_token() {
    let hooks = DefaultLaunchHooks::default();
    let listener = std::net::TcpListener::bind(("127.0.0.1", 0)).unwrap();
    let port = listener.local_addr().unwrap().port();
    drop(listener);
    ...
    hooks
        .start_helper(port, "test-token".to_string())
        .await
        .unwrap();
...
async fn launch_lifecycle_runs_sync_before_launch_writes_success_and_shutdowns_on_exit() {
    ...
    let hooks = FakeHooks::new(events.clone())
        ...
        .with_launch_result(CodexLaunch::Process {
            ...
            wait_strategy: agentkey_core::launcher::ProcessWaitStrategy::TrackedChild,
...
async fn launch_lifecycle_keeps_js_injection_in_relay_mode() {
    ...
    let events = Arc::new(Mutex::new(Vec::<String>::new()));
    let hooks = FakeHooks::new(events.clone()).with_settings(BackendSettings {
        launch_mode: agentkey_core::settings::LaunchMode::Relay,
        ..BackendSettings::default()
    });
...
async fn launch_starts_helper_when_chat_protocol_proxy_is_enabled() {
    ...
    let settings = BackendSettings {
        ...
        relay_profiles: vec![RelayProfile {
            ...
            relay_mode: agentkey_core::settings::RelayMode::MixedApi,
            official_mix_api_key: false,
            test_model: String::new(),
            config_contents: String::new(),
            auth_contents: String::new(),
            use_common_config: true,
            context_selection: agentkey_core::settings::RelayContextSelection::default(),
            context_selection_initialized: false,
            context_window: String::new(),
            auto_compact_limit: String::new(),
            model_insert_mode: agentkey_core::settings::RelayModelInsertMode::default(),
...
impl LaunchHooks for FakeHooks {
    ...
    async fn start_helper(&self, helper_port: u16, _helper_token: String) -> anyhow::Result<()> {
