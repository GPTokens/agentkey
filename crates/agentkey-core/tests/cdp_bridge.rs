use agentkey_core::assets;
use agentkey_core::bridge::{self, BRIDGE_BINDING_NAME};
use agentkey_core::cdp::{CdpTarget, list_targets, pick_page_target};
...
fn injection_script_exposes_fast_service_tier_control() {
    ...
    assert!(script.contains("data-agentkey-service-tier-controls"));
    assert!(script.contains("removeAgentKeyServiceTierBadges"));
    assert!(script.contains("installAgentKeyServiceTierDispatcherPatch"));
    assert!(script.contains("服务模式"));
    assert!(script.contains("data-agentkey-service-tier-status"));
    assert!(script.contains("data-agentkey-service-tier-inherit"));
    assert!(script.contains("data-agentkey-service-tier-standard"));
    assert!(script.contains("data-agentkey-service-tier-fast"));
    assert!(script.contains("data-agentkey-service-tier-custom"));
    assert!(script.contains("data-agentkey-service-tier-thread-inherit"));
    assert!(script.contains("data-agentkey-service-tier-thread-standard"));
    assert!(script.contains("data-agentkey-service-tier-thread-fast"));
    assert!(script.contains("global-standard"));
    assert!(script.contains("global-fast"));
    assert!(script.contains("defaultMode"));
    assert!(script.contains("agentKeyServiceTierEffectiveThreadMode"));
    assert!(script.contains("agentKeyServiceTierDefaultModeForControlMode"));
    assert!(script.contains("normalizeAgentKeyServiceTierControlMode(state.mode) !== \"custom\""));
    ...
    assert!(script.contains("agentkey-service-tier-badge"));
    assert!(script.contains("installAgentKeyServiceTierBadge"));
    assert!(script.contains("toggleAgentKeyServiceTierFromBadge"));
    assert!(script.contains("wireAgentKeyServiceTierBadge"));
    assert!(script.contains("agentKeyServiceTierBadgePlacement"));
    assert!(script.contains("agentKeyServiceTierBadgeFooterGroup"));
    assert!(script.contains("agentKeyServiceTierFindComposerEl"));
    assert!(script.contains("agentKeyServiceTierVisibleComposerFooters"));
    assert!(script.contains("agentKeyServiceTierBestComposerFooter"));
    assert!(script.contains("agentKeyServiceTierComposerCandidates"));
    assert!(script.contains("agentKeyServiceTierComposerScore"));
    assert!(script.contains("data-agentkey-service-tier-badge"));
    assert!(script.contains("agentKeyServiceTierBadgeWired"));
...
fn manager_ui_exposes_pure_api_relay_mode_button() {
    let repo = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .expect("core crate should live under crates/agentkey-core");
    let source = std::fs::read_to_string(repo.join("apps/agentkey-manager/src/App.tsx")).unwrap();
    let commands =
        std::fs::read_to_string(repo.join("apps/agentkey-manager/src-tauri/src/lib.rs")).unwrap();

    assert!(source.contains("账号兼容混入 API Key"));
    assert!(source.contains("无账号 API"));
...
async fn install_bridge_routes_binding_while_waiting_for_command_response() {
    ...
    agentkey_core::diagnostic_log::set_diagnostic_log_path_for_tests(Some(log_path.clone()));
    ...
    agentkey_core::diagnostic_log::set_diagnostic_log_path_for_tests(None);
}
