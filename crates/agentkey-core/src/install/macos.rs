...
fn copy_icon(resources: &Path) -> anyhow::Result<()> {
    let source = std::env::current_exe()
        ...
        .map(|path| path.join("agentkey.png"));
    if let Some(source) = source.filter(|path| path.exists()) {
        fs::copy(source, resources.join("agentkey.png"))?;
    }
    Ok(())
}
...
<dict>
  ...
  <string>ai.gptokens.agentkey{identifier_suffix}</string>
