...
pub const SILENT_NAME: &str = "AgentKey";
pub const MANAGER_NAME: &str = "AgentKey Manager";
pub const SILENT_BINARY: &str = "agentkey";
pub const MANAGER_BINARY: &str = "agentkey-manager";
...
pub fn shortcut_names() -> (&'static str, &'static str) {
    ("AgentKey.lnk", "AgentKey Manager.lnk")
}

pub fn app_bundle_names() -> (&'static str, &'static str) {
    ("AgentKey.app", "AgentKey Manager.app")
}
...
fn platform_install(options: &InstallOptions) -> anyhow::Result<()> {
    ...
    {
        let _ = options;
        anyhow::bail!("当前平台暂不支持安装 AgentKey 入口")
    }
}
...
fn platform_uninstall(options: &InstallOptions) -> anyhow::Result<()> {
    ...
    {
        let _ = options;
        anyhow::bail!("当前平台暂不支持卸载 AgentKey 入口")
    }
}
