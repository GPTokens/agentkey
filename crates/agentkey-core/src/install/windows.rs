...
const UNINSTALL_SUBKEY: &str = r"Software\Microsoft\Windows\CurrentVersion\Uninstall\AgentKey";
const LEGACY_UNINSTALL_SUBKEY: &str =
    r"Software\Microsoft\Windows\CurrentVersion\Uninstall\AgentKeyLegacy";
...
pub fn build_windows_entrypoint_plan(options: &InstallOptions) -> WindowsEntrypointPlan {
    ...
    WindowsEntrypointPlan {
        ...
        uninstall_key: "AgentKey".to_string(),
        legacy_uninstall_key: "AgentKeyLegacy".to_string(),
...
fn write_uninstall_registration(plan: &WindowsEntrypointPlan) -> anyhow::Result<()> {
    ...
    let install_location = Path::new(&plan.manager_path)
        ...
        .to_string();
    for (name, value) in [
        ("DisplayName", "AgentKey".to_string()),
        ("DisplayVersion", crate::version::VERSION.to_string()),
        ("Publisher", "GPTokens".to_string()),
...
fn default_icon_path() -> PathBuf {
    std::env::current_exe()
        ...
        .map(|path| path.join("agentkey.ico"))
        .unwrap_or_else(|| PathBuf::from("agentkey.ico"))
}
