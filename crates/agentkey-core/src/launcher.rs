...
pub trait LaunchHooks: Send + Sync {
    ...
    async fn inject_bridge(
        ...
        self.inject(debug_port, helper_port, helper_token).await
    }
    async fn ensure_injection(
        ...
        for attempt in 1..=120 {
            let result = match self.bridge_context(debug_port, app_dir).await {
                Ok(Some(ctx)) => {
                    self.inject_bridge(debug_port, helper_port, helper_token, ctx)
                        .await
                }
                Ok(None) => self.inject(debug_port, helper_port, helper_token).await,
                Err(error) => Err(error),
            };
            match result {
                Ok(()) => return true,
    ...
    H: IntoLaunchHooks,
{
    ...
    let helper_token = new_helper_session_token();
    ...
    let result: anyhow::Result<LaunchHandle> = async {
        ...
        if settings.enhancements_enabled {
            let injection_ready = hooks
                .ensure_injection(debug_port, helper_port, &helper_token, &app_dir)
                .await;
            if injection_ready {
                keep_launched_on_error = false;
                hooks
                    .start_bridge_watchdog(debug_port, helper_port, &helper_token)
                    .await?;
            } else {
                let degraded = launch_status(
                    "running_degraded",
                    "Codex 已启动，AgentKey 增强仍在等待页面就绪。",
                    debug_port,
                    helper_port,
                    &app_dir,
                );
...
impl LaunchHooks for DefaultLaunchHooks {
    ...
    async fn run_provider_sync(&self) -> anyhow::Result<()> {
        anyhow::bail!("provider sync requires launcher hooks with agentkey-data integration")
    }
    ...
    async fn start_helper(&self, helper_port: u16, helper_token: String) -> anyhow::Result<()> {
        ...
        let task = tokio::spawn(async move {
            let helper_token = Arc::new(helper_token);
            loop {
                tokio::select! {
                    _ = &mut shutdown_rx => break,
                    accepted = listener.accept() => {
                        if let Ok((stream, addr)) = accepted {
                            let helper_token = Arc::clone(&helper_token);
                            tokio::spawn(async move {
                                let _ = handle_helper_connection(stream, Some(addr), helper_token).await;
                            });
                        }
                    }
                }
    ...
    async fn inject(
        ...
        retry_injection(debug_port, helper_port, helper_token).await
    }
    ...
    async fn start_bridge_watchdog(
        ...
        let helper_token = helper_token.to_string();
        let task = tokio::spawn(async move {
            ...
            loop {
                tokio::select! {
                    _ = &mut shutdown_rx => break,
                    _ = interval.tick() => {
                        let _ = check_and_reinject_bridge(debug_port, helper_port, &helper_token).await;
                    }
                }
            }
        });
...
async fn handle_helper_connection(
    ...
    let cors_origin = allowed_helper_cors_origin(request_origin.as_deref());
    let proxy_path = crate::protocol_proxy::is_responses_proxy_path(path)
    ...
    if method == "OPTIONS" {
        write_http_response(
            &mut stream,
            if request_origin.is_none() || cors_origin.is_some() {
                "204 No Content"
            } else {
                "403 Forbidden"
            },
    ...
    if !helper_request_authorized(&request, helper_token.as_ref().as_str(), proxy_path) {
        let body = serde_json::to_vec(&serde_json::json!({
            "status": "failed",
            "message": "Unauthorized helper request"
        }))?;
        write_http_response(
            &mut stream,
            "401 Unauthorized",
    ...
    if crate::protocol_proxy::is_models_proxy_path(path) && method == "GET" {
...
async fn handle_models_proxy_connection(
    ...
    write_http_response(stream, &status, &content_type, &body, cors_origin).await?;
    log_helper_response(
        if is_success {
...
async fn handle_chat_completions_proxy_connection(
    ...
    if upstream.is_stream && is_success {
        write_http_stream_headers(stream, &status, &content_type, cors_origin).await?;
        let mut bytes_stream = upstream.response.bytes_stream();
        while let Some(chunk) = bytes_stream.next().await {
...
async fn write_http_response(
    ...
    let cors_headers = helper_cors_headers(cors_origin);
    let response = format!(
...
fn helper_request_authorized(request: &str, helper_token: &str, allow_relay_token: bool) -> bool {
    ...
    let bearer = http_header_value(request, "Authorization")
        .and_then(|value| bearer_token(&value))
        .filter(|value| !value.trim().is_empty());
    if bearer.as_deref() == Some(helper_token) {
        return true;
    }

    if !allow_relay_token {
        return false;
    }
    let Some(bearer) = bearer else {
        return false;
    };
    relay_proxy_bearer_tokens()
        .into_iter()
        .any(|token| token == bearer)
}
...
fn relay_proxy_bearer_tokens() -> Vec<String> {
    let settings = SettingsStore::default().load().unwrap_or_default();
    let profile = settings.active_relay_profile();
    let mut tokens = Vec::new();
    if !profile.api_key.trim().is_empty() {
        tokens.push(profile.api_key.trim().to_string());
    }
    if !settings.relay_api_key.trim().is_empty() {
        tokens.push(settings.relay_api_key.trim().to_string());
    }
    if let Ok(auth) = serde_json::from_str::<serde_json::Value>(&profile.auth_contents) {
    ...
    if let Some(token) = config_experimental_bearer_token(&profile.config_contents) {
        tokens.push(token);
    }
    tokens.sort();
    tokens.dedup();
...
fn config_experimental_bearer_token(config_contents: &str) -> Option<String> {
    let doc = config_contents.parse::<toml::Value>().ok()?;
    let provider_id = doc
        .get("model_provider")
...
fn allowed_helper_cors_origin(origin: Option<&str>) -> Option<String> {
    let origin = origin?.trim();
    if origin.is_empty() || origin == "null" {
        return None;
    }
    if helper_origin_allowed(origin) {
...
async fn retry_injection(
    ...
    for _ in 0..20 {
        match try_inject(debug_port, helper_port, helper_token).await {
            Ok(()) => return Ok(()),
            Err(error) => {
...
pub async fn check_and_reinject_bridge(
    debug_port: u16,
    helper_port: u16,
    helper_token: &str,
) -> bool {
    ...
    match retry_injection(debug_port, helper_port, helper_token).await {
        Ok(()) => {
            let _ = crate::diagnostic_log::append_diagnostic_log(
                "bridge.reinject_ok",
                serde_json::json!({
...
async fn try_inject(debug_port: u16, helper_port: u16, helper_token: &str) -> anyhow::Result<()> {
    let targets = crate::cdp::list_targets(debug_port).await?;
    let target = crate::cdp::pick_page_target(&targets)?;
    let websocket_url = target
        .web_socket_debugger_url
        .as_deref()
        .ok_or_else(|| anyhow::anyhow!("selected CDP target has no websocket URL"))?;
    let script = crate::assets::injection_script(helper_port, helper_token);
